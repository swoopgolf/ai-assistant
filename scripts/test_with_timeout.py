#!/usr/bin/env python3
"""
Timeout-based test script for the Orchestrator Agent
"""
import requests
import json
import sys
...
def quick_health_check():
    ...
    try:
        with timeout(5):
            response = requests.get("http://localhost:10200/health", timeout=3)
...
def test_agent_discovery():
    ...
    try:
        ...
        print("[ERROR] Agent discovery timed out")
        return []

def test_orchestrator_with_timeout(query, test_name, max_timeout=15):
    """Test orchestrator with strict timeout"""
    ...
    try:
        with timeout(max_timeout):
            ...
            response = requests.post(
                ...
                timeout=max_timeout - 2  # Leave 2 seconds buffer
            )
...
def main():
    ...
    print("=== Timeout-Based Orchestrator Testing ===")
    ...
    for query, test_name in test_cases:
        success, result = test_orchestrator_with_timeout(query, test_name, max_timeout=20)
    ...
    if success_count > 0:
        ...
        print("\n[CONCLUSION] System needs debugging - no successful routing")
